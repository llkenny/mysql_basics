...
CREATE TABLE IF NOT EXISTS `max_oil`.`organization` (
  ...
  `manager_id` INT UNSIGNED NOT NULL COMMENT 'Закрепленный менеджер',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `max_oil`.`contract` (
  ...
  `date` DATETIME NOT NULL COMMENT 'Дата заключение договора',
  `organization_id` INT UNSIGNED NOT NULL COMMENT 'Компания, с которой заключен договор',
  `expire_at` DATETIME NULL COMMENT 'Срок действия договора',
...
CREATE TABLE IF NOT EXISTS `max_oil`.`user` (
  ...
  `password` CHAR(32) NULL COMMENT 'Хэш пароля SHA256',
